...
export default function App() {
    ...
    const ConfigSection = () => (
        <div className="space-y-4 w-full">
            <p className="text-xs font-bold text-gray-400 uppercase text-center">Ustawienia Gry</p>
            ...
            <div className="flex flex-wrap gap-2 justify-center max-h-32 overflow-y-auto p-1">
                {Object.keys(CATEGORIES_DATA[gameState.mainCategory]).map(cat => (
                    <button key={cat} onClick={() => setGameState(p => ({ ...p, currentCategory: cat }))} className={`px-4 py-2 rounded-xl text-xs font-bold border transition-colors ${gameState.currentCategory === cat ? 'bg-indigo-600 text-white border-indigo-600' : 'bg-white text-gray-600 border-gray-100'}`}>
                        {cat.toUpperCase()}
                    </button>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 gap-2">
                ...
                {gameState.mode !== 'spelling' && (
                    <select className="w-full p-3 rounded-xl border-2 border-transparent bg-white shadow-sm font-bold text-sm" value={gameState.difficulty} onChange={(e) => setGameState(p => ({ ...p, difficulty: e.target.value as Difficulty }))}>
                        <option value="easy">Poziom: Łatwy</option>
                        <option value="normal">Poziom: Normalny</option>
    ...
    return (
        <div className="min-h-screen w-full flex items-center justify-center p-4">
            <Card className="w-full max-w-lg min-h-[650px] flex flex-col justify-between overflow-hidden">
                ...
                {view === 'teacher-lobby' && (
                    <div className="flex flex-col gap-6 items-center overflow-y-auto max-h-[600px] scrollbar-hide">
                        <div className="text-center"><p className="text-[10px] font-bold text-indigo-500 uppercase mb-1">Kod Pokoju:</p><div className="text-4xl font-black text-indigo-700 tracking-widest">{roomCode}</div></div>
                        <div className="w-full bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                            <h3 className="font-bold text-indigo-900 text-sm mb-2">Uczniowie ({Object.keys(players).length}):</h3>
                            <ul className="flex flex-wrap gap-2">{Object.values(players).map(p => (<li key={p.id} className="bg-white px-3 py-1 rounded-lg text-xs font-bold border border-indigo-200">{p.nick}</li>))}</ul>
                        </div>
                        <ConfigSection />
                        <Button disabled={!gameState.currentCategory || Object.keys(players).length === 0} onClick={() => { broadcast({ type: PeerMessageType.START_GAME, payload: { mainCategory: gameState.mainCategory, category: gameState.currentCategory, difficulty: gameState.difficulty, mode: gameState.mode } }); setView('teacher-dashboard'); }} className="w-full h-16">Rozpocznij lekcję</Button>
                        <Button onClick={handleExitToMenu} variant="ghost" className="text-xs">Wyjdź</Button>
                    </div>
                )}
                {view === 'student-join' && (
                    <div className="flex flex-col gap-6 items-center">
                        <h2 className="text-3xl font-bold text-indigo-700">Dołącz do lekcji</h2>
                        <input type="text" className="w-full p-4 rounded-xl border-2 font-bold focus:border-indigo-500 outline-none" placeholder="Twoje imię/nick" onChange={(e) => setNick(e.target.value)} />
                        <input type="text" className="w-full p-4 rounded-xl border-2 font-bold uppercase focus:border-indigo-500 outline-none" placeholder="Kod pokoju" maxLength={5} onChange={(e) => setRoomCode(e.target.value)} />
                        <Button disabled={!roomCode || !nick} onClick={() => setupStudent(roomCode, nick)} className="w-full h-16">Połącz się</Button>
                    </div>
                )}
                {view === 'student-wait' && (
                    <div className="flex flex-col gap-6 items-center text-center justify-center flex-1">
                        ...
                        <h2 className="text-2xl font-bold text-indigo-700">Jesteś w pokoju!</h2>
                        <p className="text-gray-500 text-sm">Nauczyciel wkrótce wybierze kategorię i rozpocznie grę.</p>
                        <Button onClick={handleExitToMenu} variant="ghost">Rozłącz się</Button>
                    </div>
                )}
                {view === 'teacher-dashboard' && (
                    <div className="flex flex-col gap-6 h-full">
                         <div className="flex justify-between items-center">
                            <h2 className="text-xl font-bold text-indigo-700">Wyniki na żywo</h2>
                        </div>
                        <div className="flex-1 space-y-3 overflow-y-auto">
                            {Object.values(players).map(p => (
                                <div key={p.id} className="bg-white p-3 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                                    <div className="flex-1 mr-4">
                                        <p className="font-bold text-gray-800 text-sm">{p.nick}</p>
                                        <div className="w-full h-2 bg-gray-100 rounded-full mt-2 overflow-hidden">
                                            <div className="h-full bg-emerald-500 transition-all duration-300" style={{ width: `${(p.progress / TOTAL_QUESTIONS) * 100}%` }} />
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="text-lg font-black text-indigo-600">{p.score}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {view === 'summary' && (
                    <div className="flex flex-col gap-8 text-center items-center justify-center flex-1">
                        <div className="text-8xl">🥇</div><h2 className="text-4xl font-black text-indigo-700">Koniec!</h2><p className="text-gray-500 font-bold">Zebrałeś {gameState.score} punktów!</p><Button onClick={handleExitToMenu} className="w-full h-16">Wróć do menu</Button>
                    </div>
                )}
            </Card>
        </div>
